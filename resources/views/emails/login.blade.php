    ...
    <div style="width:100%">
        <div>
            ...
            <p style="padding-bottom: 12px">
                Lien de connexion : <a href="{{ route('loginWithToken', $token) }}"
                                       target="_blank">{{ route('loginWithToken', $token) }}</a>
            </p>
            <p style="font-size:15px;line-height:1.4;margin:0 0 15px;color:#3c4149">
                Important : ce lien expire à {{ now()->addHours(2)->addMinutes(10)->format('d/m/Y à h:i') }}
            </p>
            <hr style="width:100%;border:none;border-top:1px solid #eaeaea;border-color:#dfe1e4;margin:42px 0 26px"/>
            <a href="https://dev.pbitoolbox.com/" style="color:#b4becc;text-decoration:none;font-size:14px" target="_blank">Pbi
                toolbox</a>
        </div>
    </div>
</div>
